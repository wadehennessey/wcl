...
(defvar *ansi-loop-gentemp*
	nil)

(defun ansi-loop-gentemp (&optional (pref 'loopvar-))
  (if *ansi-loop-gentemp*
      (gensym (string pref))
      (gensym)))

(defvar *loop-real-data-type* 'lisp:real)
...
(defmacro loop-collect-rplacd (;;; &environment env
  ...
  (setq form (macroexpand form nil))
...
(defun make-loop-minimax (answer-variable type)
  (let ((infinity-data (cdr (assoc type *loop-minimax-type-infinities-alist* :test #'subtypep))))
    (make-loop-minimax-internal
      ...
      :type type
      :temp-variable (ansi-loop-gentemp 'loop-maxmin-temp-)
      :flag-variable (and (not infinity-data) (ansi-loop-gentemp 'loop-maxmin-flag-))
      :operations nil
...
(defun loop-note-minimax-operation (operation minimax)
  ...
  (when (and (cdr (loop-minimax-operations minimax))
    ...
    (setf (loop-minimax-flag-variable minimax) (ansi-loop-gentemp 'loop-maxmin-flag-)))
  operation)
...
(defun ansi-loop-tequal (x1 x2)
  (and (symbolp x1) (string= x1 x2)))


(defun ansi-loop-tassoc (kwd alist)
  (and (symbolp kwd) (assoc kwd alist :test #'string=)))


(defun ansi-loop-tmember (kwd list)
...
;;;primarily for generating error messages (see loop-error, ansi-loop-warn).
...
(defun loop-constant-fold-if-possible (form &optional expected-type)
  ...
  (let ((new-form form) (constantp nil) (constant-value nil))
	...
	(ansi-loop-warn "The form ~S evaluated to ~S, which was not of the anticipated type ~S."
...
(defmacro loop-body (;;;&environment env
  ...
  (labels ((psimp (l)
    ...
    (do ((threshold (loop-code-duplication-threshold nil))) (nil)
	       ...
	       (cond ((setq tem (get fn 'estimate-code-size))
			...
			(setq n (+ n (estimate-code-size-1 (cadr l) env) 1))))
		     ...
		     ((eq fn 'function)
		      ...
		      (if (valid-function-name-p (cadr x))
			  1
...
(defun ansi-loop-warn (format-string &rest format-args)
...
(defun loop-check-data-type (specified-type required-type
  ...
  (if (null specified-type)
	...
	(cond ((not b)
	       (ansi-loop-warn "LOOP couldn't verify that ~S is a subtype of the required type ~S."
...
(defun subst-gensyms-for-nil (tree)
  ...
  (cond
    ((null tree) (car (push (ansi-loop-gentemp) *ignores*)))
    ((atom tree) tree)
      ...
      (cond ((not (symbolp keyword))
	    ...
	    (t (setq *loop-source-context* *loop-source-code*)
	       ...
	       (cond ((setq tem (loop-lookup-keyword keyword (loop-universe-keywords *loop-universe*)))
		     ...
		     ((ansi-loop-tmember keyword '(and else))
...
(defun loop-emit-final-value (&optional (form nil form-supplied-p))
  ...
  (when *loop-final-value-culprit*
    (ansi-loop-warn "LOOP clause is providing a value for the iteration,~@
...
(defun loop-disallow-aggregate-booleans ()
  (when (ansi-loop-tmember *loop-final-value-culprit* '(always never thereis))
...
(defun loop-optional-type (&optional variable)
  ...
  (and *loop-source-code*			;Don't get confused by NILs...
	 ...
	 (cond ((ansi-loop-tequal z 'of-type)
...
(defun loop-make-variable (name initialization dtype &optional iteration-variable-p)
  (cond ((null name)
	 (cond ((not (null initialization))
		(push (list (setq name (ansi-loop-gentemp 'loop-ignore-))
			    initialization)
		      *loop-variables*)
	...
	(initialization
	 (cond (*loop-destructuring-hooks*
		...
		(push (list name initialization) *loop-variables*))
	       (t (let ((newvar (ansi-loop-gentemp 'loop-destructure-)))
		    (loop-declare-variable name dtype)
...
(defun loop-maybe-bind-form (form data-type)
  (if (loop-constantp form)
      form
      (loop-make-variable (ansi-loop-gentemp 'loop-bind-) form data-type)))

		 ...
		 (cond ((not (symbolp key))
			(loop-error
			  ...
			  key for))
		       (t (setq *loop-source-context* *loop-source-code*)
			  (ansi-loop-pop-source)
			  (when (and (ansi-loop-tequal (car *loop-source-code*) 'it)
				     first-clause-p)
	       ...
	       (if (ansi-loop-tequal (car *loop-source-code*) :and)
		   (ansi-loop-pop-source)
		   (return (if (cdr body) `(progn ,@(nreverse body)) (car body)))))))
      (let ((then (get-clause for))
	    (else (when (ansi-loop-tequal (car *loop-source-code*) :else)
		    (ansi-loop-pop-source)
		    (list (get-clause :else)))))
	(when (ansi-loop-tequal (car *loop-source-code*) :end)
	  (ansi-loop-pop-source))
...
(defun loop-get-collection-info (collector class default-type)
  (let ((form (loop-get-form))
	...
	(name (when (ansi-loop-tequal (car *loop-source-code*) 'into)
		(ansi-loop-pop-source)
		(ansi-loop-pop-source))))
...
(defun loop-list-collection (specifically)	;NCONC, LIST, or APPEND
  (multiple-value-bind (lc form) (loop-get-collection-info specifically 'list 'list)
	...
	(setf (loop-collector-tempvars lc)
	      (setq tempvars (list* (ansi-loop-gentemp 'loop-list-head-)
				    (ansi-loop-gentemp 'loop-list-tail-)
  ...
  (multiple-value-bind (lc form)
	...
	(setf (loop-collector-data lc)
	      (setq data (make-loop-minimax
			   (or (loop-collector-name lc) (ansi-loop-gentemp 'loop-maxmin-))
...
(defun loop-do-with ()
  ...
  (do ((var) (val) (dtype)) (nil)
    (setq var (ansi-loop-pop-source)
	  dtype (loop-optional-type var)
	  val (cond ((ansi-loop-tequal (car *loop-source-code*) :=)
		     (ansi-loop-pop-source)
		     (loop-get-form))
		    (t nil)))
...
(defun loop-hack-iteration (entry)
  (flet ((make-endtest (list-of-forms)
	...
	(loop-error "Iteration in LOOP follows body code."))
      ...
      (unless (ansi-loop-tequal (car *loop-source-code*) :and)
	...
	(loop-bind-block)
	(return nil))
      (ansi-loop-pop-source)				; flush the "AND"
...
(defun loop-do-for ()
  (let* ((var (ansi-loop-pop-source))
	 (data-type (loop-optional-type var))
	 (keyword (ansi-loop-pop-source))
	 (first-arg nil)
	 (tem nil))
...
(defun loop-do-repeat ()
  ...
  (let ((form (loop-get-form))
	(type 'lisp:real))
    (let ((var (loop-make-variable (ansi-loop-gentemp) form type)))
...
(defun loop-when-it-variable ()
  (or *loop-when-it-variable*
      (setq *loop-when-it-variable*
	    (loop-make-variable (ansi-loop-gentemp 'loop-it-) nil nil))))

...
(defun loop-ansi-for-equals (var val data-type)
  ...
  (cond ((ansi-loop-tequal (car *loop-source-code*) :then)
...
(defun loop-for-across (var val data-type)
  (loop-make-iteration-variable var nil data-type)
  (let ((vector-var (ansi-loop-gentemp 'loop-across-vector-))
	(index-var (ansi-loop-gentemp 'loop-across-index-)))
	...
	(if (and (consp vector-form) (eq (car vector-form) 'the))
      ...
      (let* ((length 0)
	     (length-form (cond ((not constantp)
				 (let ((v (ansi-loop-gentemp 'loop-across-limit-)))
...
(defun loop-list-step (listvar)
  ...
  (let ((stepper (cond ((ansi-loop-tequal (car *loop-source-code*) :by)
			(ansi-loop-pop-source)
			(loop-get-form))
		       (t '(function cdr)))))
    (cond ((and (consp stepper) (eq (car stepper) 'quote))
	   (ansi-loop-warn "Use of QUOTE around stepping function in LOOP will be left verbatim.")
	   (values `(funcall ,stepper ,listvar) nil))
	  ((and (consp stepper) (eq (car stepper) 'function))
	   (values (list (cadr stepper) listvar) (cadr stepper)))
	  (t (values `(funcall ,(loop-make-variable (ansi-loop-gentemp 'loop-fn-) stepper 'function)
			       ,listvar)
		     nil)))))
  ...
  (multiple-value-bind (list constantp list-value) (loop-constant-fold-if-possible val)
    (let ((listvar var))
      (cond ((and var (symbolp var)) (loop-make-iteration-variable var list data-type))
	    (t (loop-make-variable (setq listvar (ansi-loop-gentemp)) list 'list)
	       (loop-make-iteration-variable var nil data-type)))
...
(defun loop-for-in (var val data-type)
  (multiple-value-bind (list constantp list-value) (loop-constant-fold-if-possible val)
    (let ((listvar (ansi-loop-gentemp 'loop-list-)))
      (loop-make-iteration-variable var nil data-type)
...
(defun loop-for-being (var val data-type)
  ...
  (let ((path nil)
    ...
    (cond ((ansi-loop-tmember val '(:each :the)) (setq path (ansi-loop-pop-source)))
	  ((ansi-loop-tequal (car *loop-source-code*) :and)
	   (ansi-loop-pop-source)
	   (setq inclusive t)
	   (unless (ansi-loop-tmember (car *loop-source-code*) '(:its :each :his :her))
	     (loop-error "~S found where ITS or EACH expected in LOOP iteration path syntax."
			 (car *loop-source-code*)))
	   (ansi-loop-pop-source)
	   (setq path (ansi-loop-pop-source))
...
(defun named-variable (name)
  (let ((tem (ansi-loop-tassoc name *loop-named-variables*)))
    (declare (list tem))
    (cond ((null tem) (values (ansi-loop-gentemp) nil))
...
(defun loop-collect-prepositional-phrases (preposition-groups &optional USING-allowed initial-phrases)
  (flet ((in-group-p (x group) (car (ansi-loop-tmember x group))))
    (do ((token nil)
	...
	(when (setq this-prep (in-group-p token group))
      ...
      (cond (this-group
	    ...
	    ((and USING-allowed (ansi-loop-tequal token 'using))
	     (ansi-loop-pop-source)
	     (do ((z (ansi-loop-pop-source) (ansi-loop-pop-source)) (tem)) (nil)
	       (when (cadr z)
		 (if (setq tem (ansi-loop-tassoc (car z) *loop-named-variables*))
		     (loop-error
...
(defun ansi-loop-sequencer (indexv indexv-type indexv-user-specified-p
   ...
   (let ((endform nil)				;Form (constant or variable) with limit value.
	 ...
	 ((:upto :to :downto :above :below)
	  (cond ((ansi-loop-tequal prep :upto) (setq inclusive-iteration (setq dir ':up)))
		((ansi-loop-tequal prep :to) (setq inclusive-iteration t))
		((ansi-loop-tequal prep :downto) (setq inclusive-iteration (setq dir ':down)))
		((ansi-loop-tequal prep :above) (setq dir ':down))
		((ansi-loop-tequal prep :below) (setq dir ':up)))
	  (setq limit-given t)
	  ...
	  (setq endform (if limit-constantp
			    `',limit-value
			    (loop-make-variable
			      (ansi-loop-gentemp 'loop-limit-) form indexv-type))))
	 (:by
	   (multiple-value-setq (form stepby-constantp stepby)
	     (loop-constant-fold-if-possible form indexv-type))
	   (unless stepby-constantp
	     (loop-make-variable (setq stepby (ansi-loop-gentemp 'loop-step-by-)) form indexv-type)))
	 (t (loop-error
     ...
     (cond ((member dir '(nil :up))
	    (when (or limit-given default-top)
	      (unless limit-given
		(loop-make-variable (setq endform (ansi-loop-gentemp 'loop-seq-limit-))
				    nil indexv-type)
...
(defun ansi-loop-sequence-elements-path (variable data-type prep-phrases
...
(defun loop-hash-table-iteration-path (variable data-type prep-phrases &key which)
  ...
  (let ((ht-var (ansi-loop-gentemp 'loop-hashtab-))
	(next-fn (ansi-loop-gentemp 'loop-hashtab-next-))
	...
	(named-variable (if (eq which 'hash-key) 'hash-value 'hash-key))
      ...
      (let* ((key-var nil)
	     (val-var nil)
	     (temp-val-var (ansi-loop-gentemp 'loop-hash-val-temp-))
	     (temp-key-var (ansi-loop-gentemp 'loop-hash-key-temp-))
	     (temp-predicate-var (ansi-loop-gentemp 'loop-hash-predicate-var-))
	     (variable (or variable (ansi-loop-gentemp)))
	...
	(when (consp key-var)
	  (setq post-steps `(,key-var ,(setq key-var (ansi-loop-gentemp 'loop-hash-key-temp-))
			     ,@post-steps))
	  (push `(,key-var nil) bindings))
	(when (consp val-var)
	  (setq post-steps `(,val-var ,(setq val-var (ansi-loop-gentemp 'loop-hash-val-temp-))
			     ,@post-steps))
...
(defun loop-package-symbols-iteration-path (variable data-type prep-phrases &key symbol-types)
  ...
  (let ((pkg-var (ansi-loop-gentemp 'loop-pkgsym-))
	(next-fn (ansi-loop-gentemp 'loop-pkgsym-next-))
	(variable (or variable (ansi-loop-gentemp)))
...
;;;INTERFACE: ANSI
(defmacro ansi-loop (;;;&environment env
