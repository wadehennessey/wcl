...
(eval-when (compile load eval)
  (defun wcl-make-symbol (name)
    ;; PCL use using make-symbol doesn't work well with static compilation.
    (gentemp name))
  )
...
(defun with-augmented-environment-internal (env functions macros)
  (let ((new-fenv (loop for (function-name . stuff) in functions
			collect (cons function-name stuff)))
	(new-menv
	 (loop for (macro-name walker-macro-info) in macros
	       collect
	       (cons macro-name
		     (lisp::make-pcl-walker-macro
		      :expansion-function (etypecase walker-macro-info
					    (list (first walker-macro-info))
					    (function walker-macro-info))
		      :info walker-macro-info)))))
    ;; Why do we need to add functions to a macro env????
    ;; HEY! discared new-fenv for now...if we need it, need to package
    ;; it up inside a structure, else expand-macro barfs. I don't think
    ;; fenv is ever use, though, since ENVIRONMENT-FUNCTION never seems
    ;; to get called.
    (append new-menv env)))

;;; Why do we need this?
(defun environment-function (env fn)
  (warn "HEY! environment-function ~A ~A" env fn)
  nil)

(defun environment-macro (env macro)
  (let ((expander (cdr (assoc macro env :test #'eq))))
    (typecase expander
      (lisp::pcl-walker-macro (lisp::pcl-walker-macro-info expander))
      (lisp::macro (lisp::macro-expander-function macro))
      (t nil))))
...
(defmacro with-augmented-environment
	  ((new-env old-env &key functions macros) &body body)
  (let ((funs (wcl-make-symbol "FNS"))
	(macs (wcl-make-symbol "MACROS"))
	(new  (wcl-make-symbol "NEW")))
    `(let ((,funs ,functions)
	   (,macs ,macros)
	   (,new ()))
       (dolist (f ,funs)
...
(defmacro with-new-definition-in-environment
  ...
  (let ((functions (wcl-make-symbol "Functions"))
	(macros (wcl-make-symbol "Macros")))
    `(let ((,functions ())
	   (,macros ()))
...
(defun convert-macro-to-lambda (llist body &optional (name "Dummy Macro"))
  (let ((gensym (wcl-make-symbol name)))
...
(defun convert-macro-to-lambda (llist body &optional (name "Dummy Macro"))
  (si:defmacro-1
    'sys:named-lambda 'sys:special (wcl-make-symbol name) llist body))
...
#+WCL
(progn
  (define-walker-template lisp::named-function             (NIL NIL CALL))
  (define-walker-template lisp::named-macro-function       (NIL NIL CALL))
  (define-walker-template lisp::mv-bind walk-multiple-value-bind)
  )
