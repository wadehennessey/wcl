...
(lisp::defun-inline fin-ref (fin i)
  (lisp::%32bit-ref fin i))

(lisp::defun-inline set-fin-ref (fin i value)
  (lisp::%32bit-def fin i value)
  value)

(defsetf fin-ref set-fin-ref)

(lisp::defun-inline funcallable-instance-p (x)
  (and (lisp::%tag= x lisp::type-procedure)
       (lisp::%eq (lisp::%object-length x) lisp::funcallable-instance-flag)))

(defun default-funcallable-instance-function (&rest args)
  (error "Default funcallable instance handler called with args ~{~A ~}"
	 args))

(defun allocate-funcallable-instance-1 ()
  (let ((fin (lisp::alloc_words 3 lisp::type-procedure)))
    (lisp::set-procedure-type-flag fin lisp::funcallable-instance-flag)
    (set-funcallable-instance-function fin
				       #'default-funcallable-instance-function)
    (setf (fin-ref fin 1) nil)
    (setf (fin-ref fin 2) nil)
    fin))
...
(lisp::defun-inline copy-procedure-code-pointer (dest-proc src-proc)
  (lisp::%32bit-def dest-proc 0 (lisp::%32bit-ref src-proc 0)))

(defun set-funcallable-instance-function (fin new-procedure)
  (if (funcallable-instance-p fin)
      (cond ((lisp::procedurep new-procedure)
	     (copy-procedure-code-pointer fin new-procedure))
	    ((symbolp new-procedure)
	     (copy-procedure-code-pointer fin
					  (make-trampoline new-procedure)))
	    (t (error "~A is not a function" new-procedure)))
      (error "~A is not a funcallable-instance" fin))
  fin)

(defmacro funcallable-instance-data-1 (fin data-name)
  (if (lisp::quoted-constant-p data-name)
      `(fin-ref ,fin ,(1+ (funcallable-instance-data-position
			   (second data-name))))
      (error "data-name ~A is not a constant" data-name)))
