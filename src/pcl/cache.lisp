...
(defun emit-fetch-wrapper (metatype argument dest miss-label &optional slot)
  (let ((exit-emit-fetch-wrapper (wcl-make-symbol "exit-emit-fetch-wrapper")))
    (with-lap-registers ((arg t))
      (ecase metatype
	(standard-instance
	  (let ((get-std-inst-wrapper (wcl-make-symbol "get-std-inst-wrapper"))
		(get-fsc-inst-wrapper (wcl-make-symbol "get-fsc-inst-wrapper")))
	    (flatten-lap
	...
	(class
	  ...
	  (let ((get-std-inst-wrapper (wcl-make-symbol "get-std-inst-wrapper"))
		(get-fsc-inst-wrapper (wcl-make-symbol "get-fsc-inst-wrapper"))
		(get-built-in-wrapper (wcl-make-symbol "get-built-in-wrapper")))
	    (flatten-lap
	      (opcode :move (operand :arg argument) arg)
	...
	(built-in-instance
	  (when slot (error "Can't do a slot reg for this metatype."))
	  (let ((get-built-in-wrapper (wcl-make-symbol "get-built-in-wrapper")))
	    (flatten-lap
	      (opcode :move (operand :arg argument) arg)
	      (opcode :built-in-instance-p arg get-built-in-wrapper)
	      (opcode :go miss-label)
