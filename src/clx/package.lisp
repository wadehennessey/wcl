...
(export
  '(*version* access-control access-error access-hosts
    ...
    warp-pointer-relative warp-pointer-relative-if-inside
  ...
  ;; HEY! shouldn't have to do this - in package above should be in
  ;; effect.
  (lisp::find-or-make-package "XLIB")
  )


(defvar *clx-version* #.(lisp::library-version (lisp::lookup-library :clx)))

(defvar *clx-build-date* #.(with-output-to-string (x) (print-time :stream x)))
